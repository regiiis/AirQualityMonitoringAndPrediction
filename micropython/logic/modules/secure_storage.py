...
class SecureStorage:
    ...
    Attributes:
        ...
        API_KEY_MIN_LENGTH (int): Minimum required API key length
        API_KEY_MAX_LENGTH (int): Maximum allowed API key length
    ...
    def _validate_api_key(self, api_key: str):
        ...
        Raises:
            ValueError: If API key fails validation
        """
        ...
        if len(api_key) < self.API_KEY_MIN_LENGTH:
            raise ValueError(
                f"API key must be at least {self.API_KEY_MIN_LENGTH} characters"
            )

        if len(api_key) > self.API_KEY_MAX_LENGTH:
            raise ValueError(
                f"API key must be {self.API_KEY_MAX_LENGTH} characters or less"
            )
    ...
    def get_api_key(self):
        ...
        Returns:
            str: API key if successful, None if not found or error
        ...
        api_key_buffer = None
        try:
            api_key_buffer = bytearray(self.API_KEY_MAX_LENGTH)
            ...
            # Extract API key before clearing buffer
            ...
            # Ensure buffer is cleared even if an error occurs
            if api_key_buffer:
                self._secure_clear(api_key_buffer)
    ...
    def prompt_and_store_api_key(self):
        """
        Prompt user for API key and store it securely.

        Returns:
            bool: True if API key was successfully stored, False otherwise

        Note:
            Provides feedback about API key requirements to user
        """
        try:
            print(
                f"API key must be between {self.API_KEY_MIN_LENGTH} and {self.API_KEY_MAX_LENGTH} characters"
            )
            ...
            print(f"Error in API key input: {e}")
            return False
